...
public class LoginGoogleServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (new DAO().isEmailExists(googleAccount.getEmail())) {
            ...
            System.out.println(acc.getAcc_email());
            acc.setAcc_password(pass);
            acc.setAcc_dob(dob);
            acc.setAcc_gender(gender);
            acc.setAcc_phone(phone);
            boolean accountSaved = new DAO().saveGoogleAccount(acc.getAcc_email(), acc.getAcc_password(), acc.getAcc_fullname(), acc.getAcc_dob(), acc.getAcc_gender(), acc.getAcc_phone());
            if (accountSaved) {
                session.setAttribute("account", acc);
